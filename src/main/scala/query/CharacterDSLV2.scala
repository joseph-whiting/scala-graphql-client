...
object Client {
  ...
  class CharacterQuery[A, B](fields: AbstractCharacterFieldQuery[A])(
      inner: AbstractQuery[B])
      extends AbstractQuery[TypedWithCharacters[A, B]] {
    def generateQuery() = ""
    def parseResponse(response: String) =
      inner.parseResponse(response) :: new WithCharacters[A]()
  }

  abstract class AbstractCharacterFieldQuery[A] extends AbstractQuery[A]

  class CharacterFieldQuery[A, B](inner: AbstractQuery[A])(
      field: CharacterField[A, B])
      extends AbstractCharacterFieldQuery[B] {
    ...
    def parseResponse(response: String) =
      field.addTrait(inner.parseResponse(response))
  }
  ...
  def character[A]()(fields: AbstractCharacterFieldQuery[A]) =
    new CharacterQuery(fields)(new EmptyQuery())
  def name =
    new CharacterFieldQuery(new EmptyQuery())(new NameField[EmptyType]())
