...
class SchemaParser extends Parser {
  ...
  def parse(inputString: String): Seq[TypeDefinition] = typePattern.findAllIn(inputString).map(parseType _).toSeq
  def parseType(typeString: String): TypeDefinition = typeString match {
    case typePattern(_, name, fields) => TypeDefinition(DefinedType(name), parseFields(fields))
  }
